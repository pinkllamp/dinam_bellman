...
bool have_zero (int* mas, int size){
    bool all_used = true;
...
int* bellman(int**catalog, int m, int Y, int y, int i){
    ...
    if (y == 0 or i==-1) return best_x;   //F(0) = 0 
        ...
        for (int xi = 1; xi<x_max+1; xi++){     //для всех допустимых 1<=xi<=Y/wi
            res_bellman = bellman(catalog, m, Y,y-xi*catalog[i][1],i-1);
            f = catalog[i][0]*xi + res_bellman[0];    //Рекуррентное выражение Беллмана
            if (f>max_f and have_zero(res_bellman, i)) {  //ищем max {ci*xi + fi-1(yi - wi*xi)} с условием что испольщуются все СрЗИ
                best_x[0] = f;    
                max_f = f;                 
                for (int j=1; j<i+1; j++)
        ...
        if (!have_zero(best_x, i+1)){
            for (int j = 0; j < i+2; j++) best_x[j] = 0;
        }
        return best_x;
    }
    delete[] best_x;
...
int main (){
    ...
    for (int j = 0; j<m; j++ ){
        ...
        for (int y = 0; y < Y+1; y+=delta_y){
            ...
            if (!have_zero(res_bellman, j+1)){
                cout << setw(4) << y << setw(7) << "-    ";
                for (int i=1; i<j+2; i++) cout << setw(3) << '-';
            }else{
